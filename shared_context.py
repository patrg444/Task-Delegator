...
class SharedContext:
    ...
    def get_github_repo(self) -> Optional[Dict[str, str]]:
        """Get GitHub repository information"""
        context = self._read_json_safe(self.context_file)
        if context.get('github_repo'):
            return {
                'url': context['github_repo'],
    ...
    def update_worker_status(self, worker_id: str, status: str, task_id: Optional[str] = None):
    ...
    def send_message(self, from_worker: str, to_worker: Optional[str], message: Dict[str, Any]):
    ...
    def get_messages(self, worker_id: str, mark_read: bool = True) -> List[Dict[str, Any]]:
        ...
        for msg_file in self.messages_dir.glob("*.json"):
            try:
                ...
                if (msg_data['to'] is None or msg_data['to'] == worker_id) and \
                   worker_id not in msg_data['read_by']:
...
class GitHubProjectManager:
    ...
    def create_github_repo(self, repo_name: str, description: str = "", private: bool = True) -> Dict[str, str]:
...
class WorkerCommunicator:
    ...
    def sync_code_changes(self, workspace_path: str, message: str):
        ...
        self.github_manager.sync_worker_changes(
            self.worker_id,
            workspace_path,
            message
        )
