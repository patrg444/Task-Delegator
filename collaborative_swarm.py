...
from shared_context import SharedContext, GitHubProjectManager, WorkerCommunicator
...
"""
        ...
        if result.get('status') == 'success':
            self.communicator.broadcast_message('task_completed', {
                'task_id': task.get('id'),
                'worker': self.name,
    ...
    async def setup_project(self, project_name: str, description: str = ""):
        ...
        try:
            ...
            repo_info = self.github_manager.create_github_repo(
                project_name,
                description or f"Collaborative project managed by Claude swarm",
                private=True
            )
...
"""
            ...
            subprocess.run(["git", "push", "-u", "origin", "main"], cwd=repo_info['path'], check=True)
    ...
    async def merge_worker_branches(self):
        ...
        worker_branches = [b for b in branches if any(w in b for w in self.workers.keys())]
            ...
            try:
                ...
                pr_result = subprocess.run(
                    [
                        ...
                        "--body", f"Automated merge of worker branch {branch_name}"
                    ],
                    cwd=repo_info['path'],
                    capture_output=True,
                    text=True
...
async def main():
    ...
    project_info = await manager.setup_project(
        "collaborative-demo",
        "Demo project for collaborative Claude development"
    )
