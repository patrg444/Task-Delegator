...
class Task:
    ...
    def __init__(self, task_id: str, prompt: str, task_type: str = "general", 
                 priority: int = 5, weight: float = 1.0):
        self.id = task_id
        self.prompt = prompt
        self.type = task_type
...
def choose_worker_count(tasks: List[Task], max_workers: int) -> int:
    ...
    logger.info(f"Workload analysis: {len(tasks)} tasks, total weight: {total_weight:.1f}, "
...
async def worker(
            ...
            for attempt in range(retry_count):
                try:
                    ...
                    if attempt < retry_count - 1:
                        logger.warning(f"[{name}] Retry {attempt + 1}/{retry_count} for task {task.id}: {exc}")
                        ...
                        logger.error(f"[{name}] Failed task {task.id} after {retry_count} attempts: {exc}")
    ...
    logger.info(f"[{name}] Shutting down. Stats: {worker_stats['tasks_completed']} completed, "
...
async def orchestrate(tasks: List[Task]) -> Tuple[Dict[str, Task], Dict[str, Any]]:
    ...
    coros = [
        worker(name, cfg_dir, queue, results, stats)
        for name, cfg_dir in chosen_accounts
    ]
