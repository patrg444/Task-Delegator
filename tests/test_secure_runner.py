...
from task_delegator.secure_runner import SecureClaudeRunner, PolicyEnforcer
...
class TestSecureClaudeRunner:
    ...
    async def test_shell_injection_prevention(self, mock_subprocess):
        ...
        mock_proc.communicate.assert_called_once_with(
            input=malicious_prompt.encode('utf-8')
        )
...
class TestPolicyEnforcer:
    ...
    def test_blocked_patterns(self):
        ...
        config = {
            'blocked_patterns': ['rm -rf', 'sudo', 'password']
        }
    ...
    def test_confirmation_patterns(self):
        ...
        config = {
            'require_confirmation': ['delete', 'remove', 'drop']
        }
        enforcer = PolicyEnforcer(config)
    ...
    async def test_on_prompt_hook(self):
        ...
        # Safe prompt
        proceed, modified = await enforcer.on_prompt(
            "Calculate 2+2",
            "worker_1"
        )
