...
class WorkerStats:
    ...
    def record_success(self, execution_time: float):
        ...
        self.average_task_time = self.total_execution_time / (self.tasks_completed + self.tasks_failed)
    ...
    async def worker_loop(self, worker_id: str, config_dir: Path):
        ...
        while True:
            try:
                ...
                allowed, modified_prompt = await self.policy.on_prompt(
                    task.prompt, worker_id
                )
    ...
    async def orchestrate(self, tasks: List[Task]) -> Dict[str, Any]:
        ...
        for worker_id, config_dir in workers.items():
            worker_task = asyncio.create_task(
                self.worker_loop(worker_id, config_dir)
            )
