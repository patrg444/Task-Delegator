...
class MetricsCollector:
    ...
    def __init__(self, window_size: int = 100):
        ...
        self.worker_metrics: Dict[str, Dict[str, Any]] = defaultdict(lambda: {
            'tasks_completed': 0,
            'tasks_failed': 0,
            'total_time': 0.0,
            'last_active': None
        })
        self.task_type_metrics = defaultdict(lambda: {
            'count': 0,
            'success': 0,
            'failed': 0,
            'avg_time': 0.0
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance': {
                'avg_execution_time': sum(self.task_times) / len(self.task_times) if self.task_times else 0,
    ...
    def render(self) -> str:
        ...
        if rate_limits['recent_events'] > 0:
            ...
            if rate_limits['last_event']:
                output.append(f"Last Event: {rate_limits['last_event']['worker_id']} "
...
class MonitoringServer:
    ...
    async def start(self):
        ...
        self.server = await asyncio.start_server(
            self.handle_metrics, '127.0.0.1', self.port
        )
...
async def example_with_monitoring():
    ...
    # Create tasks
    tasks = [
        Task(f"task_{i}", f"Process item {i}", TaskType.GENERAL)
        for i in range(10)
    ]
    ...
    print(f"Metrics available at: http://localhost:8888/")
