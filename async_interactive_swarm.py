...
class InteractiveSwarmOrchestrator:
    ...
    def choose_worker_count(self, tasks: list[Task]) -> int:
        ...
        logger.info(
            f"Workload: {len(tasks)} tasks, weight: {total_weight:.1f}, "
    ...
    async def monitor_workers(self):
        ...
        while self.stats["completed"] + self.stats["failed"] < self.stats["total_tasks"]:
            ...
            # Log status
            logger.info(
                f"Progress: {self.stats['completed']}/{self.stats['total_tasks']} completed, "
    ...
    async def orchestrate(self, tasks: list[Task]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        self.stats["tasks_per_minute"] = (
            (self.stats["total_tasks"] / duration) * 60 if duration > 0 else 0
        )
    ...
    async def run_tasks(self, tasks: list[Task]) -> dict[str, Any]:
        ...
        summary = {
            ...
            "automation_stats": {
                ...
                "automation_rate": (
                    stats["auto_continued"]
                    / (stats["auto_continued"] + stats["manual_interventions"])
                    * 100
                    if stats["auto_continued"] + stats["manual_interventions"] > 0
                    else 100
                ),
            },
            "performance": {
...
async def main():
    ...
    tasks = [
        Task(
            "impl_1",
            "Implement a Python function to calculate fibonacci numbers with memoization",
            "implementation",
            priority=8,
            weight=2,
        ),
        Task(
            "debug_1",
            "Debug and fix the authentication middleware that's causing 401 errors",
            "debugging",
            priority=9,
            weight=3,
        ),
